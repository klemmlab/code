function handles = MeasureObjectAreaShapeVolume(handles)

% Help for the Measure Object Area Shape Volume module:
% Category: Measurement
%
% SHORT DESCRIPTION:
% Adapation of the MeasureObjectAreaShape module of CellProfiler version 1
% https://cellprofiler.org/previous_releases/
% Adapated to also calculate the "Volume" (i.e. feature number 11) from the
% area measurment. The Volume measurment is an approximation of the real volume
% of the object and assume the object is a perfect sphere.
% Measures several area and shape features of identified objects. Calculate the
% volume from the area measurement
%
% COMMENT
% The module should be used after IdentifyLDs to measure the properties of the segmented
% lipid droplets (LDs).
% *************************************************************************
%
...
% MinorAxisLength         |       9
% Orientation             |      10
% Volume                  |      11 % Do not use with Zernike
%
...
for i = 1:length(ObjectNameList)
    ...
    if  NumObjects > 0
        ...
        % Volume
        Volume = ((cat(1,props.Area) ./pi).^(3./2))*(4./3).*pi;
